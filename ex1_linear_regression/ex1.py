...
def f_0(x, theta_0, theta_1, read_y):
    return theta_0 + theta_1 * x - read_y


def f_1(x, theta_0, theta_1, read_y):
    return (theta_0 + theta_1 * x - read_y) * x
...
def drew_J_theta():
    ...
    for i in range(len(theta_0_list)):
        for j in range(len(theta_1_list)):
            ...
            print("theta_0: {t0}, theta_1: {t1}, cost: {c}".
                  format(t0=theta_0_list[i], t1=theta_1_list[j], c=cost[i, j]))
